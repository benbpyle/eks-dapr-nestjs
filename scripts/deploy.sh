...
COMMS_IMAGE="public.ecr.aws/f8u4w2p3/node/comms"
GREETER_IMAGE="public.ecr.aws/f8u4w2p3/node/greeter"
...
main() {
    ...
    if ! docker info &> /dev/null; then
        ...
        print_status "On macOS, you can start Docker with: open -a Docker"
        exit 1
    fi
    ...
    if [ -z "$DATADOG_API_KEY" ]; then
        print_error "DATADOG_API_KEY environment variable is required."
        print_status "Get your API key from: https://app.datadoghq.com/organization-settings/api-keys"
        print_status "Export it with: export DATADOG_API_KEY='your-api-key-here'"
        exit 1
    fi

    print_success "All prerequisites met!"

    # Step 1: Create EKS Cluster with Nodegroup
    print_status "Step 1: Creating EKS cluster with managed nodegroup..."
    if eksctl get cluster --name $CLUSTER_NAME --region $REGION &> /dev/null; then
        print_warning "Cluster $CLUSTER_NAME already exists, skipping creation..."
    else
        print_status "Creating EKS cluster and nodegroup using cluster-config.yaml..."
        print_status "This may take 15-20 minutes for complete setup..."

        if eksctl create cluster -f kubernetes/cluster-config.yaml; then
            print_success "EKS cluster and nodegroup created successfully!"
        else
    ...
    # Verify cluster and nodes are ready
    print_status "Verifying cluster connection and nodes..."
    for i in {1..60}; do
        if kubectl get nodes &> /dev/null; then
            ready_nodes=$(kubectl get nodes --no-headers | grep -c " Ready " || echo "0")
            if [ "$ready_nodes" -ge 1 ]; then
                print_success "Cluster connection verified with $ready_nodes ready nodes"
                break
            fi
        fi
        ...
        print_status "Waiting for nodes to become Ready... ($i/60)"
        sleep 10
    done

    # Step 1b: Associate OIDC Provider (CRITICAL for EBS CSI Driver and service accounts)
    print_status "Step 1b: Associating OIDC provider for IAM service accounts..."
    ...
    # Step 2: Install EBS CSI Driver (CRITICAL for Dapr scheduler)
    print_status "Step 2: Installing EBS CSI Driver..."
    ...
    if ! eksctl get addon --cluster $CLUSTER_NAME --region $REGION | grep aws-ebs-csi-driver &> /dev/null; then
        ...
        for i in {1..60}; do
            if kubectl get pods -n kube-system -l app=ebs-csi-controller &> /dev/null; then
                pod_count=$(kubectl get pods -n kube-system -l app=ebs-csi-controller --no-headers | wc -l)
                if [ "$pod_count" -gt 0 ]; then
                    print_status "Found $pod_count EBS CSI controller pods, waiting for them to be ready..."
                    break
                fi
            fi

            if [ "$i" -eq 60 ]; then
                print_warning "EBS CSI driver pods not found after 5 minutes, continuing anyway..."
                break
            fi

            print_status "Waiting for EBS CSI driver pods to be created... ($i/60)"
            sleep 5
        done
        ...
        if kubectl get pods -n kube-system -l app=ebs-csi-controller &> /dev/null; then
            ...
            print_warning "EBS CSI driver pods not found, but continuing deployment..."
        fi
    else
    ...
    print_status "Step 3: Installing AWS Load Balancer Controller..."
    ...
    # Step 4: Install Datadog Operator
    print_status "Step 4: Installing Datadog Operator..."
    ...
    # Create Datadog secret from environment variable
    print_status "Creating Datadog secret from environment variable..."
    ...
    aws ecr-public create-repository --region us-east-1 --repository-name node/comms &>/dev/null || print_status "node/comms repository already exists"
    aws ecr-public create-repository --region us-east-1 --repository-name node/greeter &>/dev/null || print_status "node/greeter repository already exists"

    # Build and push NestJS comms service (with ARM64 architecture)
    print_status "Building NestJS comms service with explicit ARM64 architecture..."
    cd services/comms

    # Use multiarch build for platform compatibility
    if docker buildx build --platform linux/arm64 -t ${COMMS_IMAGE}:latest --push .; then
        print_success "NestJS comms service built and pushed successfully"
    else
        print_error "Failed to build/push NestJS comms service"
        exit 1
    fi
    cd ../..

    # Build and push Node service (with ARM64 architecture)
    print_status "Building Node.js service with explicit ARM64 architecture..."
    cd services/greeter

    # Use multiarch build for platform compatibility
    if docker buildx build --platform linux/arm64 -t ${GREETER_IMAGE}:latest --push .; then
    ...
    echo "  - ${COMMS_IMAGE}:latest"
    echo "  - ${GREETER_IMAGE}:latest"
